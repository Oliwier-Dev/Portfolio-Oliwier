...
assert.match(prompt, /currently studies Teknikprogrammet.*at Polhemskolan/i);
assert.match(prompt, /every distinct part/i);
assert.match(prompt, /You are not Oliwier/i);
assert.match(prompt, /third person/i);
assert.match(prompt, /80–150 words/i);
assert.match(prompt, /3–5 concise bullet points/i);
assert.match(prompt, /at least three bullets/i);
assert.match(prompt, /Never use Markdown tables/i);
assert.match(prompt, /Never dump the full profile/i);
assert.match(prompt, /Role & focus/i);
assert.match(prompt, /tell me about Oliwier/i);
assert.match(prompt, /exact shape.*3–4 concise bullets/i);
assert.match(prompt, /Never shorten, split, or respell/i);
assert.match(prompt, /asked about a project/i);
assert.match(prompt, /asked about hiring or website services/i);
assert.match(prompt, /asked about The Quiet Advantage/i);
assert.match(prompt, /no more than two links/i);
assert.match(prompt, /Do not end every answer with a question/i);
