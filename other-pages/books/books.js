...
function renderBook(text, target) {
  ...
  target.innerHTML = blocks.map((block) => {
    ...
    if (lines.length === 1 && chapterNames.has(first)) return `<h2>${safe[0]}</h2>`;
    if (lines.length === 1 && specialNames.has(first)) return `<h4>${safe[0]}</h4>`;
    if (lines.length === 1 && /^[“"‘']/.test(first)) return `<blockquote>${safe[0]}</blockquote>`;
    if (lines.length === 1 && first.length < 92 && !/[.!?]$/.test(first)) return `<h3>${safe[0]}</h3>`;
